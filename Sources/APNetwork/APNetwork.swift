...
public class APNetwork {
    ...
    private func createURLRequest(request: APRequest) throws -> URLRequest {
        ...
        case .get:
            urlRequest.url = try URLComponents
                .urlWithQueryParameters(url: url,
                                        parameters: request.parameters)
        default:
            urlRequest.httpBody = try request.encoder!
                .encode(parameters:  request.parameters)
        }
    ...
    private func _call<T:Decodable>(urlSession:URLSession ,
                                    urlRequest: URLRequest,
                                    onCompletion: @escaping (Result<T,APError>) -> Void) {
        urlSession
